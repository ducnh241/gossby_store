...
class OSC_Editor_Textbox extends OSC_Editor_Abstract {
    ...
    public function packEditorElements($parent_node) {
        ...
        foreach ($xpath->query('.//*[contains(@class,"osc-editor-textbox")]', $parent_node) as $element) {
            ...
            try {
                $content_html = OSC::core('editor', 'editor_textbox_cleaner')->config($this->_editor->getConfig())->clean($this->_editor->nodeGetInnerHtml($content_node));
            } catch (Exception $ex) {
                continue;
            }
            ...
            $this->_editor->nodeAddSafeClass($element, array('osc-editor-textbox'));
