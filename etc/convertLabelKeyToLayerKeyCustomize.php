...
while ($row = $DB->fetchArray()) {
    ...
    foreach ($customize_datas as $idx => $customize_data) {
        if (isset($customize_data['value'])) {
            $customize_data['label_key'] = $customize_key[$row['customize_id']][$customize_data['title']];
                ...
                if (isset($customize_data['value']['selected'])) {
                    ...
                    if (isset($customize_data['value']['components'])) {
                        foreach ($customize_data['value']['components'] as $cp => $component) {
                            $customize_data['value']['components'][$cp]['label_key'] = $customize_key[$row['customize_id']][$component['title']];
                            ...
                            unset($customize_data['value']['components'][$cp]['label_key']);
                        }
                    }
                }
            }
            unset($customize_data['label_key']);
        } else {
            if (!is_array($customize_data)) {
                $customize_data['label_key'] = $customize_key[$row['customize_id']][$customize_data];
                ...
                if (isset($customize_data['selected'])) {
                    if (isset($customize_data['components'])) {
                        foreach ($customize_data['components'] as $cp => $component) {
                            $customize_data['components'][$cp]['label_key'] = $customize_key[$row['customize_id']][$component];
                            $customize_data['components'][$cp]['layer_key'] = $customize_key[$row['customize_id']][$component];
                            if (isset($component['title'])) {
                                $customize_data['components'][$cp]['label_key'] = $customize_key[$row['customize_id']][$component['title']];
                                $customize_data['components'][$cp]['layer_key'] = $customize_key[$row['customize_id']][$component['title']];
                                if (isset($component['value']['title'])) {
                                $customize_data['components'][$cp]['label_key'] = $customize_key[$row['customize_id']][$component['value']['title']];
                                $customize_data['components'][$cp]['layer_key'] = $customize_key[$row['customize_id']][$component['value']['title']];
                                }
                                unset($customize_data['components'][$cp]['label_key']);
                            }
                        }
                    }
                }
    ...
    try {
        $DB->query("UPDATE osc_catalog_item_customize_design SET customize_data = :customize_data WHERE record_id = {$record_id} Limit 1", ['customize_data' => OSC::encode($data)],$record_id );
