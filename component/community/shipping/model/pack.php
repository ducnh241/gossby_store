 ...
 */
class Model_Shipping_Pack extends OSC_Database_Model {

    /**
     *
     * @var string
     */
    protected $_table_name = 'shipping_pack_rate';
    ...
    const SHIPPING_PACK_DYNAMIC = 1;
    const SHIPPING_PACK_STATIC = 0;
    ...
    protected function _beforeSave() {
        ...
        foreach (['shipping_method_id'] as $key) {
            if (isset($data[$key])) {
        ...
        if (isset($data['pack_key'])) {
            $data['pack_key'] = trim($data['pack_key']);

            if ($data['pack_key'] == '') {
                $errors[] = 'pack_key is empty';
            }
        }
        ...
        if (isset($data['rate_type'])) {
            $data['rate_type'] = intval($data['rate_type']);

            if (!in_array($data['rate_type'], [0, 1])) {
                $errors[] = 'rate_type is error';
            }
        }
    ...
    protected function _preDataForSave(&$data) {
        ...
        foreach (['location_data', 'location_parsed', 'quantity_rate', 'dynamic_rate'] as $key){
            if (isset($data[$key])) {
    ...
    public function getShippingPackByLocationData($product_type_ids = [], $product_type_variant_ids = [], $country_id = '', $province_id = ''){
        ...
        $DB->select('id, pack_key, shipping_method_id, location_parsed, product_type_id, product_type_variant_id, rate_type, quantity_rate, dynamic_rate',
            OSC::model('shipping/pack')->getTableName(),
            ...
            null, null, 'fetch_rate_pack');

        return $DB->fetchArrayAll('fetch_rate_pack');
    }
}
