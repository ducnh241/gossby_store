...
class Helper_PersonalizedDesign_DataValidator extends OSC_Object {
    ...
    public function validate(&$data) {
        ...
        foreach ($data as $k => $v) {
            if (!in_array($k, ['document', 'objects', 'bbox', 'image_data'], true)) {
                unset($data[$k]);
            }
        }
    ...
    protected function _validatePersonalized_switcher($object_type, &$config, &$indexes, $layer_name = '') {
        ...
        foreach ($config['options'] as $option_key => $option) {
            if (!is_array($option) || !isset($option['label']) || !isset($option['data'])) {
            ...
            if ($config['default_option_key'] == $option_key) {
                $option['data'] = [];
            } else if (!is_array($option['data'])) {
                throw new Exception('Personalized switcher option [' . $option_key . '] :: data format is incorrect');
            } else if (count($option['data']) > 0 || !isset($option['data']['ratio']) || !isset($option['data']['objects']) || !is_array($option['data']['objects'])) {
                $option['data']['ratio'] = floatval($option['data']['ratio']);

                foreach (array_keys($option['data']['objects']) as $_object_idx) {
                    try {
                        $this->_validateObject($option['data']['objects'][$_object_idx], $indexes);
                    } catch (Exception $ex) {
        ...
        foreach ($config['groups'] as $group_key => $group) {
            ...
            foreach ($group['images'] as $image_key => $image) {
                ...
                if ($image_key == $config['default_key']) {
                    ...
                    $image['data'] = [];
                } else if (!is_array($image['data']) || !isset($image['data']['ratio']) || !isset($image['data']['type_data'])) {
                    throw new Exception('Personalized image selector group [' . $group_key . '] - image [' . $image_key . '] :: data format is incorrect');
                } else {
                    $image['data']['ratio'] = floatval($image['data']['ratio']);
...
//        }
    ...
    protected function _validateBlendMode(&$type_data) {
        ...
        //COMMENT FOR NOT USING BLENDMODE ANYMORE
        // $type_data['blend_mode'] = trim($type_data['blend_mode']);

        // if (!in_array($type_data['blend_mode'], ['multiply', 'screen', 'overlay', 'darken', 'lighten', 'color-dodge', 'color-burn', 'difference', 'exclusion', 'hue', 'saturation', 'color', 'luminosity'], true)) {
        //     unset($type_data['blend_mode']);
        // }
    }
