...
class Cron_Catalog_Campaign_CreateMockupCampaignTool extends OSC_Cron_Abstract {
    public function process($params, $queue_added_timestamp) {
        ...
        while ($counter < $limit && $count_error < 4) {
            ...
            $DB->select('*', $model->getTableName(), "`queue_flag` = 1 AND `action` = 'createMockupCampaignTool'", '`added_timestamp` ASC, `queue_id` ASC', 1, 'fetch_queue');
                    ...
                    if (count($timestamp_old) == 0 || count($timestamp_new) == 0 || md5(OSC::encode($timestamp_old[$variants_printTemplates['print_template_id']])) != md5(OSC::encode($timestamp_new[$variants_printTemplates['print_template_id']]))) {
                        ...
                        foreach ($mockup_rel_collection as $_model_mockup_rel) {
                            ...
                            $data_queue[] = [
                                ...
                                'version' => $version,
                                'mockup_type' => 'store',
                                'mockup_category' => Helper_Catalog_Campaign_Mockup::CATEGORY_MOCKUP['tool_rerender']
                            ];
                        }
                    }
                }
                ...
                if (count($data_queue) > 0) {
                    foreach ($data_queue as $queue) {
                        OSC::model('catalog/product_bulkQueue')->setData([
                            ...
                            'member_id' => $member_id,
                            'action' => 'renderCampaignMockupTool',
                            'queue_data' => $queue
                        ])->save();
                    }

                    for ($i = 1; $i <= 10; $i ++) {
                        OSC::core('cron')->addQueue('catalog/campaign_renderMockupTool', null, ['ukey' => 'catalog/renderCampaignMockupTool:' . $i, 'requeue_limit' => -1, 'estimate_time' => 60 * 20]);
                    }
                }
