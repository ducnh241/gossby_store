 ...
 */
class Model_Catalog_Order_ProcessV2 extends Abstract_Core_Model {
    const _DB_BIN_READ = 'db_master';
    const _DB_BIN_WRITE = 'db_master';
    protected $_table_name = 'catalog_order_process_v2';
    ...
    protected function _beforeSave() {
        ...
        if (isset($data['line_items'])) {
            if (!is_array($data['line_items']) || count($data['line_items']) < 1) {
                $errors[] = 'Process V2 items is empty';
            } else {
                $line_items = [];
                $data['quantity'] = 0;

                foreach ($data['line_items'] as $line_item_id => $line_item) {
                    if (!isset($line_item['process_quantity']) || !isset($line_item['before_quantity'])) {

                    }

                    $line_item_id = intval($line_item_id);
                    $line_item['process_quantity'] = intval($line_item['process_quantity']);
                    ...
                    if ($line_item['process_quantity'] < 1) {
                        $errors[] = 'Line items data is incorrect';
                        break;
                    }

                    if ($line_item['process_quantity'] > $line_item['before_quantity']) {
                        $errors[] = 'Line items data is incorrect';
                        break;
                    }
                    ...
                    if (isset($line_items[$line_item_id])) {
                        ...
                        $data['quantity'] += $line_item['process_quantity'];
                    }
                }
