(function ($) {
    ...
    var OSC_EDITOR_ROOT_PATH = '';

    $(document).find('script').each(function () {
        var js_src = $(this).attr('src');

        if (!js_src) {
            return;
        }

        if ((/\/+osc_editor\.js(\?.*)?$/i).test(js_src)) {
            OSC_EDITOR_ROOT_PATH = js_src.replace(/^(.+)\/+osc_editor\.js(\?.*)?$/i, '$1');
            return false;
        }
    });
    ...
    function OSC_Editor(node, config) {
        this._initialize = function (node, config) {
            ...
            this.scroller = $(this.scroller ? this.scroller : window);
        ...
        this.inline_popupbar_command_data = 'bold italic underline | anchor link | align_left align_center align_right';
        ...
        this._inlineMode_renderSidebar = function () {
            ...
            if (cmds.length > 0) {
                var sidebar_li = $('<li />').append($('<i />').addClass('fa fa-plus')).appendTo(sidebar_ul);
            ...
            if (cmds.length > 0) {
                var sidebar_li = $('<li />').append($('<i />').addClass('fa fa-bars')).appendTo(sidebar_ul);
        ...
        this._boxMode_setBarsPosition = function () {
            ...
            if (this._box_topbar) {
                if (editarea_boundary.top >= scroller_boundary.top) {
                    this._box_topbar.removeAttr('style');
                } else {
                    this._box_topbar.offset({
                        top: $(window).scrollTop() + Math.min(scroller_boundary.top, this._editarea[0].getBoundingClientRect().bottom - 100),
        ...
        this._renderCommandBtn = function (cmd) {
            ...
            if (cmd.sub) {
                ...
                var cmd_sub_toggler_icon = document.createElement('i');
                cmd_sub_toggler_icon.className = 'fa fa-caret-down';

                cmd_sub_toggler.appendChild(cmd_sub_toggler_icon);
        ...
        this._renderCommandBtnSubList = function (cmds) {
            ...
            for (var x = 0; x < cmds.length; x++) {
                ...
                if (cmd.icon) {
                    var icon = document.createElement('i');
                    icon.className = cmd.icon;
                    cmd_node.appendChild(icon);
                }

                if (cmd.sub) {
                    var sub_toggler_icon = document.createElement('i');
                    sub_toggler_icon.className = 'fa fa-caret-right';

                    li.appendChild(sub_toggler_icon);
        ...
        this._getCommonAncestor = function () {
            ...
            for (var i = 0; i < arguments.length; i++) {
                ...
                while ((common_ancestor_node.compareDocumentPosition(node) & Node.DOCUMENT_POSITION_CONTAINED_BY) !== Node.DOCUMENT_POSITION_CONTAINED_BY) {
        ...
        this._getHighlightColor = function () {
            ...
            do {
                if (this._nodeIsEditarea(ancestor[0])) {
                    break;
                }

                if (ancestor[0].nodeName.toLowerCase() !== 'span') {
                    continue;
                }

                bg_color = ancestor.css('background-color');

                if (!bg_color) {
                    continue;
                }

                var matches = bg_color.match(/^\s*rgba?\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(,\s*(\d+)\s*)?\)\s*$/i);

                if (matches) {
                    bg_color = '#' + (matches[1] & 0xFF).toString(16).str_pad(2, 0) + (matches[2] & 0xFF).toString(16).str_pad(2, 0) + (matches[3] & 0xFF).toString(16).str_pad(2, 0);
                }
        ...
        this._contextHighlight = function () {
            return this._getHighlightColor() === this.highlight_color.toLowerCase();
        };
        ...
        this._commandHighlight = function () {
            this._execStandardCommand('hiliteColor', false, this._getHighlightColor() === this.highlight_color.toLowerCase() ? 'transparent' : this.highlight_color);
        };
        ...
        this._elementControlAlign_RenderCommand = function (node, custom_config) {
            ...
            var left_cmd = $('<li />').html($('<i />').addClass('lh-icon-multipart-align-left'))
            ...
            if (align_state.align === 'left') {
                if (!config.level_enable) {
                    ...
                    left_cmd.html($('<i />').addClass('lh-icon-multipart-align-left-more'));
            ...
            var center_icon = 'lh-icon-multipart-align-center';
            ...
            if (!align_state.align && (config.overflow_mode || config.full_mode)) {
                if (config.overflow_mode && !align_state.overflow_mode && !align_state.full_mode) {
                    center_icon = 'lh-icon-multipart-align-overflow';
                } else if (config.full_mode && !align_state.full_mode && (align_state.overflow_mode || !config.overflow_mode)) {
                    center_icon = 'lh-icon-multipart-align-full';
                }
            }

            var center_cmd = $('<li />').html($('<i />').addClass(center_icon))
                    .mousedown(function (e) {
                        e.stopPropagation();
            ...
            var right_cmd = $('<li />').html($('<i />').addClass('lh-icon-multipart-align-right'))
                    .mousedown(function (e) {
                        e.stopPropagation();
            ...
            if (align_state.align === 'right') {
                if (!config.level_enable) {
                    ...
                    right_cmd.html($('<i />').addClass('lh-icon-multipart-align-right-more'));
        ...
        this._elementControlZoom_RenderCommand = function (node, custom_config) {
            ...
            var zoom_in_cmd = $('<li />').append($('<i />').addClass('fa fa-search-plus')).click(function (e) {
                e.stopPropagation();
            ...
            var zoom_out_cmd = $('<li />').append($('<i />').addClass('fa fa-search-minus')).click(function (e) {
                e.stopPropagation();
        ...
        this._elementControlDelete_RenderCommand = function (node) {
            ...
            return [$('<li />').append($('<i />').addClass('fa fa-trash'))
                        .mousedown(function (e) {
                            e.stopPropagation();
                            self._historyAdd();
                            node.parentNode.removeChild(node);
        ...
        this._elementControlCopy_RenderCommand = function (node) {
            ...
            return [$('<li />').append($('<i />').addClass('fa fa-copy'))
                        .mousedown(function (e) {
                            e.stopPropagation();
        ...
        this._elementControlEdit_RenderCommand = function (node, config) {
            ...
            return [$('<li />').append($('<i />').addClass('fa fa-pencil'))
                        .mousedown(function (e) {
                            e.stopPropagation();
        ...
        this._elementControlCaption_RenderCommand = function (node) {
            ...
            return [$('<li />').append($('<i />').addClass('fa fa-cc'))
                        .mousedown(function (e) {
                            e.stopPropagation();
        ...
        this._elementControlCaption_RenderForm = function (node) {
            ...
            $('<label />').attr('for', '').html('Điền nội dung caption').appendTo(container);
            ...
            win = this._renderWindow('Thêm/sửa caption', container);
        };
        ...
        this._context_data = {};
        this._commands = {
            fullscreen: {cmd: 'fullscreen', icon: 'fa fa-arrows-alt'},
            bold: {cmd: 'bold', context: 'bold', icon: 'fa fa-bold'},
            italic: {cmd: 'italic', context: 'italic', icon: 'fa fa-italic'},
            underline: {cmd: 'underline', context: 'underline', icon: 'fa fa-underline'},
            strike: {cmd: 'strikeThrough', context: 'strikeThrough', icon: 'fa fa-strikethrough'},
            anchor: {cmd: 'anchor', context: 'anchor', icon: 'fa fa-bookmark-o'},
            link: {cmd: 'link', context: 'link', icon: 'fa fa-link'},
            unlink: {cmd: 'unlink', context: 'link', icon: 'fa fa-unlink'},
            align_left: {cmd: 'align', cmd_arg: 'left', context: 'justifyLeft', icon: 'fa fa-align-left'},
            align_center: {cmd: 'align', cmd_arg: 'center', context: 'justifyCenter', icon: 'fa fa-align-center'},
            align_right: {cmd: 'align', cmd_arg: 'right', context: 'justifyRight', icon: 'fa fa-align-right'},
            align_justify: {cmd: 'align', cmd_arg: 'justify', context: 'justifyFull', icon: 'fa fa-align-justify'},
            subscript: {cmd: 'subscript', context: 'subscript', icon: 'fa fa-subscript'},
            clearFormat: {cmd: 'removeFormat', icon: 'fa fa-eraser'},
            superscript: {cmd: 'superscript', context: 'superscript', icon: 'fa fa-superscript'},
            quote: {cmd: 'convertBlock', cmd_arg: 'blockquote', context: 'blockname', context_arg: ['blockquote'], icon: 'fa fa-quote-left'},
            code: {cmd: 'convertBlock', cmd_arg: 'pre', context: 'blockname', context_arg: ['pre'], icon: 'fa fa-code'},
            ul: {cmd: 'list', cmd_arg: false, context: 'list', context_arg: [false], icon: 'fa fa-list-ul'},
            ol: {cmd: 'list', cmd_arg: true, context: 'list', context_arg: [true], icon: 'fa fa-list-ol'},
            image: {cmd: 'image', context: 'image', icon: 'fa fa-image'},
            video: {cmd: 'video', icon: 'fa fa-video-camera'},
            hr: {cmd: 'hr', icon: 'osc-editor-icon-line'},
            highlight: {cmd: 'highlight', cmd_arg: '#fff8af', context: 'highlight', icon: 'osc-editor-icon-highlight'},
            heading: {cmd: 'convertBlock', cmd_arg: 'h4', context: 'blockname', context_arg: ['h4'], icon: 'fa fa-header'},
            heading_small: {cmd: 'convertBlock', cmd_arg: 'h5', context: 'blockname', context_arg: ['h5'], icon: 'osc-editor-icon-heading-small'},
            heading_medium: {cmd: 'convertBlock', cmd_arg: 'h3', context: 'blockname', context_arg: ['h3'], icon: 'osc-editor-icon-heading-medium'},
            heading_large: {cmd: 'convertBlock', cmd_arg: 'h1', context: 'blockname', context_arg: ['h1'], icon: 'osc-editor-icon-heading-large'},
            paragraph: {cmd: 'paragraph', context: 'blockname', context_arg: ['p'], icon: 'fa fa-paragraph'},
            heading_list: {cmd: 'convertBlock', cmd_arg: 'h1', context: 'blockname', context_arg: [['h1', 'h2', 'h3', 'h4', 'h5', 'h6']], icon: 'fa fa-header', sub: [
                    {cmd: 'convertBlock', cmd_arg: 'h1', context: 'blockname', context_arg: ['h1'], icon: 'fa fa-header osc-editor-h1', label: 'Heading 1'},
                    {cmd: 'convertBlock', cmd_arg: 'h2', context: 'blockname', context_arg: ['h2'], icon: 'fa fa-header osc-editor-h2', label: 'Heading 2'},
                    {cmd: 'convertBlock', cmd_arg: 'h3', context: 'blockname', context_arg: ['h3'], icon: 'fa fa-header osc-editor-h3', label: 'Heading 3'},
                    {cmd: 'convertBlock', cmd_arg: 'h4', context: 'blockname', context_arg: ['h4'], icon: 'fa fa-header osc-editor-h4', label: 'Heading 4'},
                    {cmd: 'convertBlock', cmd_arg: 'h5', context: 'blockname', context_arg: ['h5'], icon: 'fa fa-header osc-editor-h5', label: 'Heading 5'},
                    {cmd: 'convertBlock', cmd_arg: 'h6', context: 'blockname', context_arg: ['h6'], icon: 'fa fa-header osc-editor-h6', label: 'Heading 6'}
                ]},
            screen: {cmd: 'screen', cmd_arg: 'mobile-portrait', context: 'screen', icon: 'fa fa-mobile', sub: [
                    {cmd: 'screen', cmd_arg: 'mobile-portrait', context: 'screen', context_arg: 'mobile-portrait', icon: 'fa fa-mobile', label: 'Mobile Portrait'},
                    {cmd: 'screen', cmd_arg: 'mobile-landscape', context: 'screen', context_arg: 'mobile-portrait', icon: 'fa fa-mobile', label: 'Mobile Portrait'},
                    {cmd: 'screen', cmd_arg: 'tablet-portrait', context: 'screen', context_arg: 'tablet-portrait', icon: 'fa fa-tablet', label: 'Tablet Portrait'},
                    {cmd: 'screen', cmd_arg: 'tablet-landscape', context: 'screen', context_arg: 'tablet-landscape', icon: 'fa fa-tablet', label: 'Tablet Portrait'},
                    {cmd: 'screen', cmd_arg: 'pc-1024', context: 'screen', context_arg: 'pc-1024', icon: 'fa fa-television', label: 'Monitor: 1024'},
                    {cmd: 'screen', cmd_arg: 'pc-1280', context: 'screen', context_arg: 'pc-1280', icon: 'fa fa-television', label: 'Monitor: 1280'},
                    {cmd: 'screen', cmd_arg: 'pc-1600', context: 'screen', context_arg: 'pc-1600', icon: 'fa fa-television', label: 'Monitor: 1600'},
                    {cmd: 'screen', cmd_arg: 'custom', context: 'screen', context_arg: 'custom', icon: 'fa fa-cog', label: 'Custom width'}
                ]}
        };
