(function ($) {
    ...
    window.initAddressFrmHandler = function () {
        var frm = $(this);
            ...
            if (key === 'province') {
                ...
                input.bind('verify.verify', function () {
                    if (input.val().trim().replace(/\s{2,}/i, ' ').split(' ').length < 2) {
                        input.trigger('error', ['Your name must include both first and last names']);
                    }
                });
            } else {
                input.bind('verify.verify', function () {
        ...
        var __provinceRenderer = function () {
            ...
            var province_container = frm.find('[data-frm="province"] .mrk-province-input');
            ...
            if (typeof PROVINCE_LOADED[country_code] === 'undefined') {
                ...
                }).addClass('styled-input').appendTo($('<div />').appendTo(province_container));
            } else {
                var select = $('<select />').attr({
                    name: input_name,
                    id: input_id
                }).prependTo($('<div />').addClass('styled-select').append($('<ins />')).appendTo(province_container));
