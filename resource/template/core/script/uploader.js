(function ($) {
    ...
    function OSC_Uploader(node, config) {
        this._initialize = function (node, config) {
            ...
            if (typeof this.xhrFields !== 'object') {
                this.xhrFields = {};
            }
            ...
            if (this.image_mode || !this.extensions) {
            ...
            $(document).bind('dragover', function (e) {
                ...
                self._dragEnterListener(e);
            }).bind('dragleave', function (e) {
                e.preventDefault();
                e.stopPropagation();

                self._dragLeaveListener(e);
            }).bind('dragDropRelease dragend', function (e) {
                self._container.removeClass('dragdrop-active');
                self._container.removeClass('drag-entered');
            });

            this._drag_drop_area.bind('dragover dragenter', function (e) {
                e.preventDefault();
                e.stopPropagation();
            }).bind('drop', function (e) {console.log('DROP');
                e.preventDefault();
                e.stopPropagation();
                
                self._dropListener(e);
                
                $(document).trigger('dragDropRelease');
            });
        };
        ...
        this._renderInput = function () {
            ...
            this._input = $('<input />').attr({type: 'file', tabIndex: -1}).prependTo($('<div />').addClass('browser-btn').append($('<div />').addClass('input-file-label').click(function () {
                $(this).parent('.browser-btn').find('input').trigger('click')
            }).html(this.btn_content)).appendTo(this._container));
        ...
        this._dragEnterListener = function (e) {
            if (!this._container.hasClass('dragdrop-active') && this._uploaderAvailable() && this._isValidFileDrag(e)) {
                this._container.addClass('dragdrop-active');

                e.dataTransfer.dropEffect = 'none';
            }

            if (!this._container.hasClass('dragdrop-active')) {
                return;
            }

            if (this._drag_drop_area[0] === e.target || (e.target.compareDocumentPosition(this._drag_drop_area[0]) & Node.DOCUMENT_POSITION_CONTAINS)) {
        ...
        this._dragLeaveListener = function (e) {
            if (e.relatedTarget) {
                if (this._drag_drop_area[0] !== e.relatedTarget && !(e.relatedTarget.compareDocumentPosition(this._drag_drop_area[0]) & Node.DOCUMENT_POSITION_CONTAINS)) {
        ...
        this._processFiles = function (files) {
            ...
            if (this.max_files === 1 && files.length > 1) {
                alert('You can only drag & drop 1 image!');
                return false;
            }
        ...
        this._upload = function (id) {
            ...
            for (var k in this.xhrFields) {
                xhr[k] = this.xhrFields[k];
            }
        ...
        this._error = function (id, code, message) {
            console.log("Uploader ERROR: " + message + '[' + code + '][' + id + ']');
        ...
        this._validateFile = function (file) {
            ...
            if (!this._isAllowedExtension(name)) {
                ...
                this._error(null, 'maxSizeError', name);
                return false;

            } else if (size && this.min_filesize && size < this.min_filesize) {
                this._error(null, 'minSizeError', name);
                return false;
            }
