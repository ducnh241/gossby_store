...
<table style="max-width: 400px !important;<?= $params['css_attributes']['table'] ?>">
    <tr>
        <td align="center" valign="center">
            <div style="<?= $params['css_attributes']['h1'] ?>">Order Confirmation.</div>
            <div style="<?= $params['css_attributes']['message'] ?>">
                <p style="<?= $params['css_attributes']['message p'] ?>">Hi <strong><?= $order->getFirstName() ?></strong>!</p>
                <p style="<?= $params['css_attributes']['message p'] ?>">We're getting your order ready to be shipped. We will notify you when it has been sent.</p>
            </div>
        </td>
    </tr>
</table>
<?= $this->build('catalog/email_template/html/order/summary', array_merge($params, ['customer_info_render_flag' => true, 'display_estimated_price' => true])) ?>
    ...
    <tr>
        <td align="center" valign="center">
            <table class="button main-action-cell" style="<?= $params['css_attributes']['table'] ?>">
                <tr>
                    <td style="<?= $params['css_attributes']['button-cell'] ?>" align="center"><a href="<?= OSC::helper('postOffice/email')->getClickUrl($order->getDetailUrl()) ?>" style="<?= $params['css_attributes']['button-cell a'] ?>"><b>Manage or Track order</b></a></td>
