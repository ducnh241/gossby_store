    ...
    class personalizedDesignBuilder {
        ...
        _renderFrame(document_type) {
            ...
            if (this._zoom_ratio === null) {
                ...
                this._zoom_ratio = this._container.outerHeight() / document_type.height;
            ...
            var render_line_callback = '_documentTypeRenderLine_' + this.document_type.key;

            if (typeof this[render_line_callback] === 'function') {
                render_line_callback = this[render_line_callback]();
            }

            var __applyFrameRatio = function (ratio) {
                var width = $this.document_type.width * ratio;
                var height = $this.document_type.height * ratio;

                var custom_document_safe_area_width = null;
                var custom_document_safe_area_height = null;

                if ($this.document_type.custom_document_safe_area_width !== null){
                    custom_document_safe_area_width = $this.document_type.custom_document_safe_area_width * ratio;
                }

                if ($this.document_type.custom_document_safe_area_height !== null){
                    custom_document_safe_area_height = $this.document_type.custom_document_safe_area_height * ratio;
                }
                ...
                if (typeof render_line_callback === 'function') {
                    if (custom_document_safe_area_width !== null) {
                        render_line_callback(width, height, ratio, custom_document_safe_area_width, custom_document_safe_area_height);
                    }else{
                        render_line_callback(width, height, ratio);
                    }
                }
            };

            __applyFrameRatio(this._zoom_ratio);

            if(this._onRatioChange) this._svg.unbind("ratio_change", this._onRatioChange)

            this._onRatioChange = function(e, old_ratio, new_ratio, zoom_point) {
                var viewbox = $this._viewBox();
                var ratio = new_ratio / old_ratio;

                viewbox.x = (viewbox.x + zoom_point.x) * ratio - zoom_point.x;
                viewbox.y = (viewbox.y + zoom_point.y) * ratio - zoom_point.y;

                $this._svg[0].setAttribute('viewBox', viewbox.x + ' ' + viewbox.y + ' ' + viewbox.width + ' ' + viewbox.height);

                __applyFrameRatio(new_ratio);
            }
            this._svg.bind('ratio_change', this._onRatioChange);
        }
...
//                _5628x7636: $(document.createElementNS(this._svg[0].namespaceURI, 'path'))
        ...
        _documentTypeRenderLine_custom(){
            ...
            return function (width, height, ratio, custom_document_safe_area_width , custom_document_safe_area_height) {
        ...
        _zoom(percent, zoom_point) {
            percent = parseFloat(percent);

            if (isNaN(percent)) {
                percent = 1;
            }
            const old_ratio = this._zoom_ratio;

            const new_ratio = percent / 100;

            this._zoom_ratio = new_ratio;
            ...
            this._svg.trigger('ratio_change', [old_ratio, new_ratio, zoom_point]);

            this._bottom_bar.find('.zoom-bar input').val(percent);

            return percent;
        }

        _zoomIn(zoom_point) {
            var percent = this._zoom_ratio * 100;

            percent *= 1.5;

            percent = Math.max(1, percent);

            return this._zoom(percent, zoom_point);
        }
        ...
        _renderObjectSwitcher() {
            ...
            if(object.data.personalized.type === "imageSelector") {
                ...
                const onSelect = (image, key) => {
                    ...
                    this._objectApplyNewRatioToData('image', new_type_data, image.data.ratio, $this._zoom_ratio);
                ...
                const selectOption = (key, option) =>{
                    ...
                    if (!Array.isArray(option.data.objects)) {
                        option.data.objects = [];
                    }

                    this._setDataMakeObject(option.data.objects, object.elm, option.data.ratio);
...
//            return Math.sqrt(a * a + b * b);
        ...
        _commandHand_exec() {
            ...
            this._svg.bindUp('mousedown.hand-tool', function (e) {
                ...
                $this._svg.unbind('mousemove.hand-tool').unbind('mouseup.hand-tool').bind('mousemove.hand-tool', function (e) {
                    var x = viewbox.x - e.pageX + flag_point.x;
                    var y = viewbox.y - e.pageY + flag_point.y;

                    $this._svg[0].setAttribute('viewBox', x + ' ' + y + ' ' + $this._container.outerWidth() + ' ' + $this._container.outerHeight());
        ...
        _commandPenAddPoint_exec() {
            ...
            this._svg.bind('mousemove.tool-penAddPoint', function (e) {
                ...
                if (object.closest_point) {
                    ...
                    point.circle.setAttribute('stroke-width', 1);
        ...
        _objectPath_render(data, key) {
            ...
            $(path).bind('object-clearHelper', function () {
                ...
                $this._elmSetTransform(path, 0, 0, data.rotation);
            }).bind("object-remove", function() {
                $this._svg.unbind(`ratio_change.${key}`)
            });

            this._svg.unbind(`ratio_change.${key}`).bind(`ratio_change.${key}`, function (e, old_ratio, new_ratio) {
                $this._objectApplyNewRatioToData('path', data, old_ratio, new_ratio);
                $(path).trigger('object-update');
            });
        ...
        _objectPath_editorRender(object) {
            ...
            if (selected_point) {
                ...
                for (var x = 0; x < handles.length; x++) {
                    for (var y = 0; y < handles[x][1].length; y++) {
                        ...
                        handle_path.setAttribute('stroke-width', 0.5);
                        ...
                        handle_circle.setAttribute('r', 3);
            ...
            for (var i = 0; i < object.data.type_data.points.length; i++) {
                ...
                rect.setAttribute('x', point.point.x - 3);
                rect.setAttribute('y', point.point.y - 3);
        ...
        _objectText_render(data, textKey) {
            ...
            .bind('object-remove', function() {
                $this._svg.unbind(`ratio_change.${textKey}`);
            })
            ...
            this._svg.unbind(`ratio_change.${textKey}`)
                .bind(`ratio_change.${textKey}`, function (e, old_ratio, new_ratio) {
                $this._objectApplyNewRatioToData('text', data, old_ratio, new_ratio);

                $(text).trigger('object-update');
                if(data.path && typeof $this._callback.finish_action === 'function') {
...
//            data.offset *= ratio;
            ...
            $(flow_select).select2({
                ...
                ajax: {
                    ...
                    processResults: function (data, params) {
                        ...
                        console.log("params.page * limit", params.page * limit, data.totalDocs)
                        return {
        ...
        _personalizeImageSelector_Form(object) {
            ...
            const __getImageDataRelative = (imageData, image) => {
                ...
                if (tempImageData.type_data.original_size && tempImageData.type_data.size && tempImageData.type_data.position) {
                    ...
                    const {x, y, width, height} = $this._getObjectRectFitBackground(image);
                    _size = { width, height };
                    _position = { x, y };
                }
            ...
            $('<button />').addClass('btn btn-primary ml5 btn-apply-personalized').html('Apply').click(function () {
                ...
                console.log(new_type_data, data)

                $this._objectApplyNewRatioToData('image', new_type_data, images[default_key].data.ratio, $this._zoom_ratio);
        ...
        _personalizeSwitcher_FetchCallback(config, new_key) {
            ...
            $.each(config.options, function (option_key, option) {
                ...
                __applyToChildren(option.data.objects);
            });
        }
        ...
        _personalizeSwitcher_Form(object) {
            ...
            $('<button />').addClass('btn btn-primary ml5 btn-apply-personalized').html('Apply').click(function () {
                ...
                if (!Array.isArray(options[default_option_key].data.objects)) {
                    options[default_option_key].data.objects = [];
                }

                $this._setDataMakeObject(options[default_option_key].data.objects, object.elm, options[default_option_key].data.ratio);

                options[default_option_key].data = {};
        ...
        _objectText_editorRender(object) {
            ...
            $(this._objectText_makeDiv(object.data.type_data))
                    ...
                    .css({
                        position: 'absolute',
                        top: object.data.type_data.position.y - viewbox.y,
                        left: object.data.type_data.position.x - viewbox.x,
...
//            this._svg_group.defs.append($(document.createElementNS(this._svg[0].namespaceURI, 'style')).attr('type', 'text/css').append(document.createTextNode('@import url(' + font_data.css_url + ')')));
        ...
        _objectEllipse_render(data, ellipseKey) {
            ...
            $(ellipse).bind('move resize', function (e, bounding_rect) {
                ...
                $this._svg.unbind(`ratio_change.${ellipseKey}`)
            });

            this._svg.unbind(`ratio_change.${ellipseKey}`)
            .bind(`ratio_change.${ellipseKey}`, function (e, old_ratio, new_ratio) {
                $this._objectApplyNewRatioToData('ellipse', data, old_ratio, new_ratio);
        ...
        _objectRect_render(data, rectKey) {
            ...
            .bind('rotate', function (e, degress) {
                ...
                $this._svg.unbind(`ratio_change.${rectKey}`);
            ...
            this._svg.unbind(`ratio_change.${rectKey}`)
                .bind(`ratio_change.${rectKey}`, function (e, old_ratio, new_ratio) {
                    $this._objectApplyNewRatioToData('rect', data, old_ratio, new_ratio);
                    ...
                    $this._elmSetTransform(rect, 0, 0, data.rotation);
                });
        ...
        _getObjectRectFitBackground(size /* {width, height}*/) {
            //calculate current background size
            const bgBbox = {
                width: this.document_type.width * this._zoom_ratio,
                height: this.document_type.height * this._zoom_ratio,
                x: 0,
                y: 0
            }
...
//            elm.transform.baseVal.initialize(this._svg[0].createSVGTransformFromMatrix(matrix));
        ...
        _objectImage_render(data, imageKey) {
            ...
            const __setTransform = function () {
                ...
                //return [(data.size.width * $this._zoom_ratio) / data.original_size.width, 0, 0, (data.size.height * $this._zoom_ratio) / data.original_size.height, data.position.x * $this._zoom_ratio, data.position.y * $this._zoom_ratio].join(' ');
            };
...
//                }
                ...
                __setTransform();
            }).bind('apply_new_ratio', function (e, old_ratio, new_ratio) {
                e.stopPropagation();

                $this._objectApplyNewRatioToData('image', data, old_ratio, new_ratio);
                __setTransform();
            }).bind('parent_transform', function (e, scale, translate, rotation, flip, pivot) {
                e.stopPropagation();
                ...
                //clean event when remove
                $this._svg.unbind(`ratio_change.${imageKey}`)
            });

            this._svg.unbind(`ratio_change.${imageKey}`).bind(`ratio_change.${imageKey}`, function (e, old_ratio, new_ratio) {
                e.stopPropagation();

                $(image).trigger('apply_new_ratio', [old_ratio, new_ratio]);
            });
            ...
            if (object_data.personalized !== null && typeof object_data.personalized === 'object' && object_data.personalized.type === 'imageSelector') {
                $.each(object_data.personalized.config.groups, function (group_key, group) {
                    $.each(group.images, function (image_key, image) {
                        ...
                        $this._objectApplyNewRatioToData('image', image.data.type_data, image.data.ratio, $this._zoom_ratio);

                        image.data.ratio = $this._zoom_ratio;
...
//                    $this._objectGroup_applyTransformToData($this._objects[group.getAttribute('data-object')].data, scale, translate, rotation, flip, pivot);
        ...
        _objectGroup_applyTransformToData(object_data, scale, translate, rotation, flip, pivot) {
            ...
            var __applyTransformToChildren = function (ratio, children) {
                children.forEach(function (_object_data) {
                    $this._objectApplyNewRatioToData(_object_data.type, _object_data.type_data, ratio, $this._zoom_ratio);

                    var applier = '_object' + $this._objectGetTypeFunctionName(_object_data.type) + '_applyTransformToData';
                    ...
                    if (_object_data.type === 'group') {
                        __applyTransformToChildren(ratio, _object_data.type_data.children);
                    }
                });
            };
            ...
            $.each(object_data.personalized.config.options, function (option_key, option) {
                ...
                __applyTransformToChildren(option.data.ratio, option.data.objects);

                option.data.ratio = $this._zoom_ratio;
            });
        }
        ...
        _objectAdd(type, type_data, key, mode, options, ignored_auto_select = false) {
            ...
            if (typeof type_data.stroke === 'undefined') {
                type_data.stroke = {color: $('[data-cmd="stroke"]').hasClass('no-color') ? 'none' : $('[data-cmd="stroke"]').css('background-color'), width: 1};
            }
        ...
        _objectAppend(object, options, ignored_auto_select = false) {
            ...
            this._svg.bind(`ratio_change.${key}`, () => {
                if (Array.isArray(object.data.type_data.mask) && object.data.type_data.mask.length > 0 && !object.mask) {
                    __processMask();
                }
            });
        ...
        _objectLineHelperRender(boundingBox, objectPositions) {
            ...
            const __drawLine = (line, color) => {
                ...
                lineElm.setAttribute("style",`stroke:${color};stroke-width:1`)
        ...
        _objectHelperRender(object) {
            ...
            bounding.setAttribute('stroke-width', 0.5);
            bounding.setAttribute('stroke-dasharray', '5,5,5');
            object.helper.appendChild(bounding);

            //UNUSED
            // this._svg.bind('ratio_change' + helperKey, function (e, old_ratio, new_ratio) {
            //     var bounding_rect = $this._elmGetBBox(bounding);

            //     bounding_rect.x *= new_ratio / old_ratio;
            //     bounding_rect.y *= new_ratio / old_ratio;
            //     bounding_rect.width *= new_ratio / old_ratio;
            //     bounding_rect.height *= new_ratio / old_ratio;
            ...
            if (canEdit) {
                ...
                [
                    ...
                    $(object.elm).bind(`move${helperKey} resize${helperKey}`, function (e, bounding_rect) {
                        rect.setAttribute('x', point[1](bounding_rect) - 5);
                        rect.setAttribute('y', point[2](bounding_rect) - 5);
                    });
                ...
                if(object.type === 'image') {
                    ...
                    if(object.personalized?.type === "switcher" && typeof object.personalized?.config?.options === "object") {
                        for(const optionKey in object.personalized?.config?.options) {
                            ...
                            Object.assign(image_data, this._getImageDataOfObjects(option.data.objects))
                        }
                    }
                }
            })
...
//            this._doc_height = data.document.height;
            ...
            var before_ratio = this._zoom_ratio;

            this._zoom(data.document.ratio * 100);
                    ...
                    if(object.type === 'image') {
                        ...
                        if(object.personalized?.type === "switcher" && typeof object.personalized?.config?.options === "object") {
                            for(const optionKey in object.personalized?.config?.options) {
                                ...
                                if(option.data.objects) _groupImageData(option.data.objects)
                            }
                        }
                    }
                })
        ...
        _setDataMakeObject(objects, group, old_ratio) {
            var $this = this;
            ...
            for (let i = objects.length - 1; i >=0 ; i--) {
                ...
                if (typeof old_ratio !== 'undefined') {
                    $this._objectApplyNewRatioToData(object_data.type, type_data, old_ratio, $this._zoom_ratio);
                }
                ...
                if (object_data.type === 'group') {
                    $this._setDataMakeObject(object_data.type_data.children, object, old_ratio);
                }
            }
        ...
        _fetch(container, new_key, new_ratio) {
            var $this = this;
            ...
            container.find('> [data-object]').each(function () {
                ...
                if (typeof object !== 'undefined') {
                    ...
                    if (typeof new_ratio !== 'undefined') {
                        $this._objectApplyNewRatioToData(object_data.type, object_data.type_data, $this._zoom_ratio, new_ratio);
                    }

                    if (object.data.type === 'group') {
                        object_data.type_data.children = $this._fetch($(this).find('> .thumb-list'), new_key, new_ratio);
                    }
        ...
        _copyObject(object) {
            ...
            this._objectApplyNewRatioToData(object_data.type, object_data.type_data, this._zoom_ratio, 1);

            if (object.data.type === 'group') {
                object_data.type_data.children = this._fetch($(object.list_item).find('> .thumb-list'), true, 1);
            }
            ...
            try {
                ...
                toastr.error('Unable to copy: ' + e.mesage);
            }
        }
        ...
        _pasteObject() {
            ...
            if (version > 1) {
                toastr.error('Cannot copy from Design V2 to V1');
                return
            }
            ...
            this._objectApplyNewRatioToData(object_data.type, object_data.type_data, 1, this._zoom_ratio);
            ...
            if (object_data.type === 'group') {
                this._setDataMakeObject(_cloned_object_data.type_data.children, object, 1);
            }
        ...
        _pasteObjectAttributes(){
            ...
            const zoomRatio = this._zoom_ratio;
            const __toNewRatio = (n) => n / dataCopied.zoomRatio * zoomRatio;
            const bbox = {
                x: __toNewRatio(dataCopied.x),
                y: __toNewRatio(dataCopied.y),
                width: __toNewRatio(dataCopied.width),
                height: __toNewRatio(dataCopied.height),
            };
        ...
        _addNodeToGlobalNode(nodeData) {
            ...
            if (key) { // folder in already object
                ...
                if (nodeData.actionType === 'switcher') { // create a new switcher
                    ...
                    object.personalized.config.options[key].data.ratio = $this._zoom_ratio;
                    object.personalized.config.options[key].data.objects = nodeData.type_data.children;
        ...
        _getRatioFitBg(heightPSD, widthPSD) {
            this.ratioPSD = heightPSD / (this.document_type.height * this._zoom_ratio);
        }
        ...
        _findObjectByKeyInGlobalObjects(ancestors, object, isThumbnail = false, parentObject) {
            ...
            } else if (Array.isArray(object.data?.objects)) { // object in a switcher
                const childrenObject = object.data.objects?.find(object => ancestors.includes(object.key));

                object.data?.ratio && (this.parentRatio = object.data.ratio);
                if (childrenObject) {
                        ...
                        for (const optionKey in objectData.personalized.config.options) {
                            if (objectData.personalized.config.options[optionKey].label  === traceToNode[index + 1].trim()) {
                                ...
                                if (optionKey === objectData.personalized.config.default_option_key) {
                                    ...
                                    inPersonalized = true;
                                    _objects = objectData.personalized.config.options[optionKey].data.objects;
                                }
        ...
        _parseSwitcherNode(node, nodeData, dataAction) {
            ...
            if (switchers.length > 0 && (!Object.keys(dataAction).length || dataAction?.type === 'folderSwitcher' || dataAction?.isBetaRootNode === false)) {
                ...
                for (let switcher of switchers) {
                    ...
                    data.ratio = this._zoom_ratio;
                    data.objects = [];

                    switcher.children().map((node) => {
                        let parse_data = $this._parseDataFromPSD(node, dataAction);
                        if (parse_data) {
                            data.objects.push(parse_data);
                        }
                    });
                                    ...
                                    if (node.ancestors?.length == 0) { // default node
                                        ...
                                        if (key) {
                                            ...
                                            if (Array.isArray(object)) { //create
                                                ...
                                                // fix case: create image selector in option switcher but default node not true ratio
                                                $this._objectApplyNewRatioToData('image', node.type_data, $this._zoom_ratio, $this.parentRatio);
                                                // above replace below
                                                // node.type_data.size.width /= $this._zoom_ratio;
                                                // node.type_data.size.height /= $this._zoom_ratio;
                                                // node.type_data.position.x /= $this._zoom_ratio;
                                                // node.type_data.position.y /= $this._zoom_ratio;
                                                ...
                                                if (node.IsAddImageSelector) { // add image selector
                                                    ...
                                                    if (node.inPersonalized) { // in option switcher node
                                                        if (object.type === 'image') { // default image selector
                                                            $this._objectApplyNewRatioToData(object.type, object.type_data, $this._zoom_ratio, $this.parentRatio);
                                                        } else {  // option image selector
                                                            object.ratio = $this._zoom_ratio;
                                                        }
                                                    } else { // in default switcher node
                                                        object.ratio = $this._zoom_ratio;
                                                    }
                                                }
                                                ...
                                                object.type_data.position = {
                                                    x: x * ($this.parentRatio / $this._zoom_ratio),
                                                    y: y * ($this.parentRatio / $this._zoom_ratio),
                                                    // x,
                                                    // y
                                                }

                                                object.type_data.size = {
                                                    width: width * ($this.parentRatio / $this._zoom_ratio),
                                                    height: height * ($this.parentRatio / $this._zoom_ratio),
                                                    // width,
                                                    // height
                                                }
                                            }
                                    ...
                                    if (node.inPersonalizedFlag) {
                                        ...
                                        if (!node.personalized || node.personalized.type !== 'imageSelector') {
                                            ...
                                            if (key) {
                                                ...
                                                $this._objectApplyNewRatioToData(node.type, node.type_data, $this._zoom_ratio, $this.parentRatio);
                                            }
                                        }
                                    }
                                    node.type_data.id = $this._addToImageData(node.type_data);
        ...
        constructor(container, permissions) {
            ...
            new ResizeSensor(this._container[0], function () {
                ...
                $this._svg[0].setAttribute('viewBox', viewbox.x + ' ' + viewbox.y + ' ' + $this._container.outerWidth() + ' ' + $this._container.outerHeight());
            });
        ...
        if (draft_content.content !== null && typeof draft_content.content === 'object' && permissions.edit_layer) {
            if (window.confirm('You have a temporary saved data, do you want to restore?')) {
                ...
                if (meta_data?.image_folder_id_on_server?.length) {
                    builder.folderIdToUploadS3 = meta_data.image_folder_id_on_server;
                    $('#folderId').val(builder.folderIdToUploadS3);
                }
                builder.setMetaData(meta_data);
            }
        }
