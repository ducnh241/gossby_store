...
<div class="styled-select">
    <select name="<?= $name_config ?>[]" class="js-select-countries" multiple="multiple">
        <?php foreach ($countries as $country_code => $country): ?>
            <option value="<?= $country_code ?>" <?= in_array($country_code, $params['value']) ? 'selected' : '' ?>> <?= $country ?></option>
        <?php endforeach; ?>
    </select>
</div>
