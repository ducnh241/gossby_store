    ...
    <div class="loading-wrapper">
        <?php if (count($data) > 0) : ?>
            <table class="grid grid-borderless">
                ...
                ?>
                    <tr>
                        <td style="text-align: left"><a href="<?= $item['url'] ?>"><?= $item['title'] ?></a></td>
                        ...
                        <td style="text-align: left"><?= number_format($data[$product_id]['visits']) ?></td>
                        <td style="text-align: left"><?= number_format($data[$product_id]['unique_visitors']) ?></td>
                        <td style="text-align: left"><?= round(($data[$product_id]['orders'] / $data[$product_id]['unique_visitors']) * 100, 2); ?>%</td>
                        <td style="text-align: right">
                            <a class="btn btn-small btn-icon" title="View detail analytics" href="<?php echo $this->getUrl('*/*/productDetail', ['id' => $product_id, 'range' => $range]); ?>"><?= $this->getIcon('eye-regular') ?></a>
                        </td>
                    </tr>
                <?php
                    endforeach;
